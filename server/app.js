...
// Fetch compartido por GET /:genId (detalle) y GET /:genId/exportar — mismo
// header + renglones + fotos, para que ambos endpoints no puedan divergir
// silenciosamente. incluirBlobUrl=true agrega blob_url a la query de fotos
// (solo lo necesita el export, para traer el binario de Vercel Blob) — se
// omite por default para no filtrar esa URL interna en el detalle normal.
// Devuelve null si el generador no existe / no pertenece a la obra.
async function fetchGeneradorConDetalle(genId, projectId, { incluirBlobUrl = false } = {}) {
  const { rows: genRows } = await db.pool.query(`
  ...
  const { rows: fotos } = await db.pool.query(
    `SELECT id, partida, subpartida, ${incluirBlobUrl ? 'blob_url, ' : ''}nombre_archivo, subido_por, subido_en
...
app.get('/api/projects/:id/generadores-obra/:genId', h(auth.allow('residente')), h(requireProject), h(auth.verificarAccesoObra), h(auth.checkPermiso('generadores_obra', 'puede_ver')), h(async (req, res) => {
  ...
  res.json({ ...detalle.generador, renglones: detalle.renglones, fotos: detalle.fotos });
}));
...
app.get('/api/projects/:id/generadores-obra/:genId/exportar', h(auth.allow('residente')), h(requireProject), h(auth.verificarAccesoObra), h(auth.checkPermiso('generadores_obra', 'puede_ver')), h(async (req, res) => {
  ...
  if (!detalle) return res.status(404).json({ error: 'Generador no encontrado' });
  const { generador, renglones, fotos: fotosRows } = detalle;
...
// Evidencia fotográfica (Fase 2, prompt-generadores-de-obra.md) — por
// Partida/Subpartida, no por renglón individual (decisión explícita del
// prompt). Mismo candado de estado (borrador/rechazada) que los renglones,
...
app.post('/api/projects/:id/generadores-obra/:genId/fotos', h(auth.allow('residente')), h(requireProject), h(auth.verificarAccesoObra), h(auth.checkPermiso('generadores_obra', 'puede_crear')), uploadImg.single('foto'), h(async (req, res) => {
  ...
  if (check.error) {
    ...
    return res.status(check.error).json({ error: check.msg });
  }
  if (!['borrador', 'rechazada'].includes(check.gen.estado)) {
    if (req.file) await fs.promises.unlink(req.file.path).catch(() => {});
    return res.status(409).json({ error: 'Solo se pueden agregar fotos a un generador en borrador o rechazado' });
  }
  if (!req.file) return res.status(400).json({ error: 'No se recibió ninguna foto' });
  const { partida, subpartida } = req.body || {};
  if (!partida?.trim() || !subpartida?.trim()) {
    await fs.promises.unlink(req.file.path).catch(() => {});
    return res.status(400).json({ error: 'partida y subpartida son requeridos' });
  }
  ...
  const { rows } = await db.pool.query(
    `INSERT INTO generador_obra_fotos (generador_id, partida, subpartida, blob_url, nombre_archivo, subido_por)
     VALUES ($1,$2,$3,$4,$5,$6) RETURNING id, partida, subpartida, nombre_archivo, subido_por, subido_en`,
    [genId, partida.trim(), subpartida.trim(), blob.url, req.file.originalname, req.user.id]
  );
  res.status(201).json(rows[0]);
}));
...
app.get('/api/projects/:id/generadores-obra/:genId/fotos/:fotoId', h(auth.allow('residente')), h(requireProject), h(auth.verificarAccesoObra), h(auth.checkPermiso('generadores_obra', 'puede_ver')), h(async (req, res) => {
  ...
  const { rows } = await db.pool.query(
    `SELECT f.blob_url, f.nombre_archivo FROM generador_obra_fotos f
     JOIN generadores_obra g ON g.id = f.generador_id
     WHERE f.id = $1 AND f.generador_id = $2 AND g.project_id = $3 AND g.activo = true`,
    [fotoId, genId, req.project.id]
  );
...
app.delete('/api/projects/:id/generadores-obra/:genId/fotos/:fotoId', h(auth.allow('residente')), h(requireProject), h(auth.verificarAccesoObra), h(auth.checkPermiso('generadores_obra', 'puede_eliminar')), h(async (req, res) => {
  ...
  if (check.error) return res.status(check.error).json({ error: check.msg });
  if (!['borrador', 'rechazada'].includes(check.gen.estado)) {
    return res.status(409).json({ error: 'Solo se pueden eliminar fotos de un generador en borrador o rechazado' });
  }
  const { rows } = await db.pool.query('DELETE FROM generador_obra_fotos WHERE id = $1 AND generador_id = $2 RETURNING blob_url', [fotoId, genId]);
  if (!rows[0]) return res.status(404).json({ error: 'Foto no encontrada' });
  del(rows[0].blob_url).catch(() => {});
  res.json({ ok: true });
}));
