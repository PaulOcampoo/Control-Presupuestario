...
const ROLE_TABS = {
  ...
  cabo:           ['destajo', 'insumos', 'avance', 'requisiciones', 'maquinaria', 'trabajadores'],
...
// ---------------------------------------------------------------------------
// Selector estilizado reusable (customSelect) — piloto (prompt-selector-
// estilizado-piloto.md): reemplaza el <select> nativo del selector precio/
// cantidad/ambos del preview de "Actualizar presupuesto" por un trigger +
// listbox flotante propios, sin tocar ningún otro <select> de la app (ese
// rollout es un prompt aparte). El <select> real se queda oculto en el DOM
// como única fuente de verdad — mismo name/value/eventos 'change' que un
// <select> normal — para que el código que ya lee/escucha ese <select>
// (ej. actualizarEstadoBotonConfirmar en pintarPreviewActualizacionPresupuesto)
// siga funcionando sin cambios.
function enhanceSelect(select) {
  if (select.dataset.customSelectReady) return; // evita doble-enhance si se repinta el modal
  ...
  function closeList() {
    ...
    // hidden real llega después de la transición (con reduce motion activo
    // la transición dura ~0, el timeout corto no se nota).
  ...
  trigger.addEventListener('keydown', (e) => {
    ...
    // cerrando el preview completo en vez de solo el dropdown.
  ...
  // Si el valor del <select> cambia por fuera (ej. seteo directo en tests),
  // refleja el label sin necesidad de re-enhance.
...
function pintarPreviewActualizacionPresupuesto(preview, archivoUrl) {
  ...
  // Piloto de selector estilizado (prompt-selector-estilizado-piloto.md): el
  // <select> real sigue siendo la fuente de verdad (mismo .value/'change'),
  // enhanceSelect solo le agrega el trigger + listbox estilizados encima.
  selectsResolucion().forEach((s) => enhanceSelect(s));
