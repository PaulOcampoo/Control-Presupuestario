...
const PERMISSIONS = {
  ...
  // 'compradores'/'apartados' (prompt-implementacion-pr-a-compradores-
  // apartado.md, Fase 4 PR A) agregados SOLO a admin/desarrollador —
  // deliberadamente ausentes de residente/cabo/cualquier otro rol, y sin
  // entrada en permisos_usuario (Forbidden Action explícita): el único gate
  // real es este tab list + auth.allow() sin argumentos en cada endpoint de
  // server/app.js, mismo criterio que 'contrato'.
  // 'trabajadores_global'/'nominas_global' RETIRADOS de aquí (prompt-fase1-
  // fusionar-trabajadores-nominas.md): dejaron de ser tabs navegables propios
  // — 'trabajadores'/'nominas' ahora traen selector interno Esta obra/Todas
  // las obras (mismo patrón que 'finanzas' desde prompt-ajuste-finanzas-
  // selector.md), y la rama "Todas las obras" se gatea con isAdmin() en el
  // frontend, no con membresía en este array. SIGUEN existiendo como
  ...
  admin:          { label: 'Administrador', tabs: ['resumen', 'contrato', 'impuestos', 'insumos', 'requisiciones', 'ordenes', 'avance', 'programa', 'destajo', 'estadoActivo', 'presupuestoEstimaciones', 'usuarios', 'proveedores', 'cumplimiento', 'finanzas', 'compromisos', 'fondoGarantia', 'mapeo', 'trabajadores', 'nominas', 'estimaciones', 'generadoresObra', 'ordenesCambio', 'lotes', 'modelosVivienda', 'compradores', 'apartados', 'contratosVenta', 'cobranza', 'entregas', ...MAQUINARIA_TABS_ADMIN, 'cotizador', 'costos', 'costosDashboard', 'matrices', 'generadorPresupuestos', 'avance_clientes', 'composicion_costos', 'dashboardEjecutivo', 'catalogoBasicos', 'almacen'] },
  ...
  // prompt-costos-mapeo-y-mover-tiles.md: 'mapeo' agregado — primera vez que
  // este rol gana acceso a un tab fuera del set original de PR #149. 'mapeo'
  ...
  // agregado — mismo criterio que 'costos'/'composicion_costos' arriba (vista
  // GLOBAL de solo lectura, checkPermiso('costos', 'puede_ver') real dentro
  // del propio endpoint, sin sección granular nueva ni entrada en
  // TAB_A_SECCION).
  // Tarea 2 del mismo prompt: 'catalogoBasicos' agregado — mismo criterio
  // exacto que 'costosDashboard' (vista GLOBAL de solo lectura).
  costos: { label: 'Costos', tabs: ['matrices', 'generadorPresupuestos', 'costos', 'costosDashboard', 'composicion_costos', 'mapeo', 'fondoGarantia', 'programa', 'resumen', 'catalogoBasicos', 'estadoActivo'] },
};
...
const SECCIONES_PERMISOS = [
  ...
  'ordenes_cambio',
  // prompt-lotes-fase1.md: estatus de construcción de lotes/unidades
  ...
  // "informativa" en la matriz (sin checkPermiso real), mismo criterio que
  // 'estimaciones'/'programa': el gate real es auth.allow('residente') +
  // verificarAccesoObra en los endpoints, no permisos_usuario.
  'generadores_obra',
];
...
const TAB_A_SECCION = {
  ...
  // 'compromisos' (prompt-compromisos-abiertos.md) mapea a la MISMA sección
  // 'finanzas' que el tab 'finanzas' — reusa el permiso ya existente a
  // propósito (Starting State confirmado: "mismo sistema de permisos
  // existente, no whitelist nueva"), no crea una sección nueva en
  // SECCIONES_PERMISOS ni en el CHECK constraint de permisos_usuario.
  compromisos: 'finanzas',
  // 'fondoGarantia' (prompt-fondo-garantia-editable.md) mapea a la MISMA
  // sección 'finanzas' — mismo criterio y misma justificación exacta que
  // 'compromisos' arriba: reusa el permiso ya existente, no crea sección
  // nueva en SECCIONES_PERMISOS ni en el CHECK constraint de permisos_usuario.
  ...
  matrices: 'costos',
  // prompt-mover-generador-presupuestos-y-modal.md: 'generadorPresupuestos'
  // se separa de 'matrices' a su propia pestaña, pero reusa la MISMA
  // sección granular 'costos' (mismos endpoints checkPermiso('costos', ...),
  // mismo dato) — mismo criterio y misma justificación exactos que
  // 'matrices' arriba.
  generadorPresupuestos: 'costos',
...
function defaultPermisosParaRol(puesto) {
  ...
  if (puesto === 'residente') {
    ...
    // prompt-lotes-fase1.md: capacidad nueva — residente es quien captura
    // avance/programa/estimaciones de la obra día a día, rol natural para
    // reportar estatus de construcción por lote también. puede_editar=true
    // (no solo puede_crear) porque la acción principal de este módulo es
    // EDITAR el estatus de un lote ya existente, no solo dar de alta lotes
  ...
  if (puesto === 'costos') {
    ...
    // prompt-costos-mapeo-y-mover-tiles.md: acceso a Mapeo (concepto↔insumo)
    // y a "Actualizar presupuesto" — dos secciones granulares DISTINTAS pese
    // a vivir en la misma vista (ver TAB_A_SECCION.mapeo vs. los endpoints de
    // /presupuesto/actualizar/*, que checkPermiso('presupuestos', ...) en vez
    // de 'mapeo'). 'mapeo' ya tiene fila con puede_ver=true por el loop base
    ...
    // el resto de Mapeo ya funcionara. Sin fila automática por el mismo
    // motivo que 'presupuestos' arriba ('insumos' no está en
    // PERMISSIONS.costos.tabs, a propósito — costos no tiene su propio tab
    // de Insumos, solo necesita leerlo desde dentro de Mapeo).
    filas.push({
      seccion: 'insumos', puede_ver: true, puede_crear: false,
      puede_editar: false, puede_editar_precios: false, puede_eliminar: false,
    });
  }
  return filas;
